...
It **masters two subsystems**: **muses-et-oracles** (`hasard` — dice, random word/concept, fate) and **parallaxe** (`decision` — the *basculement de point de vue*). Descriptions and dialogue belong to the **narrateur** (cinerio + conversation-cards); the oracle hands its result to the narrateur, which renders it.
...
|--------|-------|-----------|
| `hasard` — dice roll, random word, random concept, unexpected event | muses-et-oracles | `R/_subsystems/muses-et-oracles/canon/` |
| `decision` — what does the world decide? what happens next? | parallaxe | `R/_subsystems/parallaxe/canon/` |
...
2. Locate the subsystem canon **with your own resolution logic** — don't depend on a single rigid path. Check, in order, until one holds the canon index file (`<nom>.md`):
   - (a) the **domain install** — `R/_subsystems/<nom>/canon/`.
   - (b) the **shared subsystem library** when the domain hasn't installed it — walk up from `R` to the **RPG-collection root** (the parent that holds the game domains), then test for `<nom>` **both** a `subsystems/<nom>/` grouping **and** a top-level `<nom>/`, each in either `…/canon/` **or** `…/systeme/canon/` form. Real layouts observed: `RPG/parallaxe/systeme/canon/` (top-level) and `RPG/subsystems/parallaxe/systeme/canon/` (grouped).
   A present-but-**empty** dir does not count — only a path holding the canon counts. If none resolves → graceful degrade (below).
...
> **Quality bar — `${CLAUDE_PLUGIN_ROOT}/skills/solo-mc/references/rebondissement-go-no-go.md`.** A drawn element is **raw matter**, not a verdict. Before handing it over, **ground it in the established fiction** (a thread / secret / front / NPC already in play) and keep it only if it passes the GO: *surprising yet inevitable*, foreshadowed (or retro-planted coherently), consequential, continuity-safe. If it can't be grounded (deus ex machina, from-nowhere, shock-for-shock) → **re-draw or discard**. The deck proposes; the checklist disposes.
...
## Parallaxe — basculement de point de vue (this oracle masters parallaxe)

Parallaxe is a `decision` subsystem: it does **not** answer yes/no — the drawn card's **Focale** (point-of-view axis) reframes the stalled decision from a new vantage, so it unblocks by *recadrage* rather than by a verdict. **You master the deck** (filtering, exclusions, Pause); after drawing (filter → draw 1 uniform), read the card and emit a **complete** directive the narrateur only has to render:

- Translate the **Focale** into the `POV →` vantage (see Output): **Moi ●** → an unbidden internal movement of the PC · **Compagnon ●●** → the companion's agency · **PNJ ○** → an NPC present drives · **Tiers ◌** → an absent figure who weighs · **Lieu ▢** → the place as an entity acts/reveals · **Pause** (#54) → statu quo, do not unblock the decision.
- Pass the card's **Phrase** (the sense), its **3 Impulsions** (*angles*, not orders — the narrateur uses one / several / none) and its **Signe** (a *type* — detail / gesture / object / word — the narrateur invents concretely in-universe). The card **does not dictate** the outcome; it recadre, and the reframe must **break linearity** (a change of angle, not a continuation of the PC's last action).

The narrateur masters the rendering (its own subsystems cinerio + conversation-cards); you master parallaxe's draw + interpretation and hand over the directive.

## Graceful degrade

If a subsystem `canon/` directory does not exist on the current machine:

- Fall back to system-appropriate dice read from `R/_systeme/canon/`.
...
If `R/_systeme/canon/` is also absent (rules not yet regenerated — see T7 in SKILL.md):

- Use the generic 2d6 oracle (result 2-6 = No, 7-9 = Yes-but, 10-12 = Yes).
- Emit: `[HRP] R/_systeme/canon/ absent — run rules-keeper before playing. Using 2d6 fallback.`
...
The system dice formula is read from `R/_systeme/canon/`. The oracle adapts its probability mapping to whatever the active system uses. The mapping rules live in the system's canon rules — never hard-coded here.
...
- **POV** (parallaxe only): an explicit *basculement de point de vue* directive derived from the card's **Focale**, so the narrateur knows which vantage to reframe from — one of: `POV → Moi: unbidden internal movement of the PC` · `POV → Compagnon: companion agency` · `POV → PNJ: NPC present drives` · `POV → Tiers: absent who weighs` · `POV → Lieu: place as entity acts/reveals`. (Pause card: `POV → — : statu quo, do not unblock the decision`.)
