...
4. Last dated session file `R/<AAAA>/<MM>/<pj>/session-*.md` — the file of maximal `<N>` per the **canonical session ordering** (`${CLAUDE_PLUGIN_ROOT}/references/jdr-layout.md › Ordre canonique des séances`), not merely "most recent year/month first match" — for recent scenes, tag changes and events
