...
- **session reports** ← **dated files** `R/<AAAA>/<MM>/<pj>/session-<AAAA-MM-JJ>-<N>.md` (one per session — **not** an aggregated `journal.md`), named and numbered per the **canonical session ordering** (`${CLAUDE_PLUGIN_ROOT}/references/jdr-layout.md › Ordre canonique des séances`). An old `journal.md` encountered during reorganization is split into dated files (one per entry, `<N>` assigned in chronological order), then archived.
