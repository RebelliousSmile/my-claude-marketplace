...
- Rules reference (terminology and mechanics): the active **game system**'s rules-keeper-optimized rules at `R/_systeme/{canon,mj}/` (official `canon/` + GM house rules `mj/`), produced by `rules-keeper`. Effective rules = canon + declared house rules. **Generic subsystems** (Parallaxe, Cinério, Muses et Oracles) are live-play tools consumed by `ttrpg:solo-mc` only — `pc` does not reference them.
