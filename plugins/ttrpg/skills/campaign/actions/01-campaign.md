...
1. **Check / bootstrap the `config.yaml`** in `R/_campagnes/<campagne>/`.
   ...
   - **If it is missing: bootstrap the campaign** (minimal bootstrap — do NOT run a long questionnaire; ask only the indispensable missing items: the setting, and the PC to attach). *(Previously delegated to `/ttrpg:solo-mc setup`; `campaign` now handles it. If `ttrpg:solo-mc` is installed, it can refine/manage the `config.yaml` later — but it is not required to start.)*
     ...
     2. Write a **minimal playable** `config.yaml`:
        ...
        # solo-mc's domain (its setup). campaign writes ONLY the identity/wiring above.
        ```
     3. **Do not write the game tuning** (`ton`, `approche`, `difficulte`, `rythme`, chaos, sub-system profiles): it belongs to **`solo-mc`** and will be filled in by its setup at play time. `campaign` limits itself to the **identity/wiring** — just enough to anchor the GM prep. Invent nothing.
...
3. **Attach the setting**: from `config.yaml › univers`, target `R/_univers/<univers>/`. If the tree does not exist, create **two thematic sub-trees** `canon/` (official lore) and `mj/` (GM creation), each with `terminologie.md`, `factions.md`, `personnages.md`, `histoire.md`, `geographie.md`. If there are raw canonical sources, propose `/ttrpg:lore-extract` to record them **into `canon/`**; the content created by the GM will go into `mj/` (via `npc`, `faction`, `scenario`).
