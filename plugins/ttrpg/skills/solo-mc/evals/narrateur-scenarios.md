...
### 2026-06-13 — run 2 (régression post-édit narrateur, dry-run, domain=monsterhearts, sans `_subsystems/`) — **5/5 PASS, 0 régression**

Vérifie que la nouvelle section **« Mechanic triggering — uncertainty → rules »** + l'étape 4 réécrite de la boucle micro-scène n'ont pas cassé la voix MJ. Domaine plus strict que la baseline 2026-06-01 (cinerio **et** conversation-cards absents → N1/N2 en dégradation gracieuse). Résultats : N1 (scène ouverte → dégrade cinerio + Scene block, finit sur question ; la section triggering ne s'active pas à l'établissement) ; N2 (PNJ hostile → dégrade conversation-cards + Dialogue block + tic ; voix MJ = réaction MC, pas de jet forcé) ; N3 (question de règle `[HRP]` → reste meta, distincte du déclenchement d'une action PJ — cohérent) ; N4 (oracle déjà résolu → rendu en fiction, non re-routé vers un jet) ; N5 (action RP → conséquence + question, le jet éventuel revient au joueur et la boucle repose une question — invariant « ne pas pré-décider le PJ » préservé). **Séparation T8 intacte** (pas de maths dans la prose), **cadence de questions intacte**, **HRP/RP non surchargé**, **MC ne lance jamais** renforcé. Aucune régression.
