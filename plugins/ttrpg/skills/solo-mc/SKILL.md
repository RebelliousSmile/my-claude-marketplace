...
- **T6** — System mechanics (oracle, dice, character creation, scene rules) come from the active system's **rules-keeper-optimized rules**, produced by `rules-keeper`, split by provenance: `canon/` (official ruleset) + `mj/` (the GM's house rules, which explicitly override canon where declared). Effective rules = canon + declared house rules; consult both, never invent mechanics. **Generic subsystems** (e.g. Parallaxe, Cinério, Muses et Oracles) are live-play tools layered on the game system (not games themselves); **`solo-mc` is their sole consumer**, at `R/_subsystems/<name>/{canon,mj}/` — produced by `rules-keeper`. The game system's own rules at `R/_systeme/{canon,mj}/` are shared with `pc` and `campaign`; subsystems are not.
