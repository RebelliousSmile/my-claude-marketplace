...
- **`user-guide`** — end-user documentation (manuals, getting-started, how-to, troubleshooting, FAQ), task-oriented and plain-language. `outline → write → review`.
- **`technical-document`** — developer/operator docs (architecture, API reference, integration guide, runbook, design note), with a per-type structure and verification of examples/links/claims against the codebase. `scope → write → verify`.
- **`specification`** — project/product specification ("cahier des charges"): objectives, explicit scope in/out, functional and non-functional requirements (unique ID + MoSCoW priority + acceptance criterion), deliverables, constraints, planning, with an adversarial challenge pass for testability and completeness. `elicit → draft → challenge`.
