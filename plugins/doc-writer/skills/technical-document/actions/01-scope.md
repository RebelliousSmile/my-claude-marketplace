...
- `subject` (required, from `$ARGUMENTS`) — what needs documenting (a system, module, API, or operation), given up front. If absent, ask for it once before proceeding.
- Access to the codebase / system.

## Process

1. **Classify the document type** against `${CLAUDE_PLUGIN_ROOT}/skills/technical-document/references/doc-types.md`: architecture, API/reference, integration, runbook, or design note. If the subject spans two, propose splitting into linked docs.
2. **Name the audience** and their goal (calling the API? operating the system? reviewing a design?) per `${CLAUDE_PLUGIN_ROOT}/references/doc-principles.md`.
