...
> Les skills d'**assemblage des intrants** — `brief` (construit `_brief/`), `forge` (concept), `research` (données) — sont dans le plugin `obsidian`.
...
| Assembler le brief / lore / données / règles de jeu | plugin `obsidian` (`brief`, `research`, `lore-extract`, `rules-keeper`) |
