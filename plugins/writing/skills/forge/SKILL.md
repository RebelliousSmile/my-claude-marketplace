...
- Use universe docs to verify consistency; flag contradictions explicitly. For a JDR-scope project, the universe docs live under `R/_univers/<univers>/{canon,mj}/` — see `ttrpg:campaign`'s domain conventions (`ttrpg`'s `references/jdr-layout.md`) for how `R` is resolved.
- If a topic lacks documentation, suggest `obs:research "<topic>"`.
