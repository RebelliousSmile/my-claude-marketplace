...
3. **Links & citations** — internal links, `file:line`/`file:symbol` references, and ADR links resolve to existing targets **and actually correspond to what they're cited for** — a citation pointing at a real but unrelated line/symbol (right file, wrong location) is a finding, not a pass.
4. **Claims** — behavioral statements match what the code actually does; flag anything outdated.
5. **Structure** — sections still match the type's template in `${CLAUDE_PLUGIN_ROOT}/skills/technical-document/references/doc-types.md`.
6. **Drift** — note anything the doc asserts that the code no longer supports.

## Outputs

The corrected document (or a findings list if review-only), with a summary of mismatches found and any claim that needs a human decision. Each mismatch or correction cites the exact `file:line`/`file:symbol` of the real source it was checked against — not just that a claim is wrong, but where the actual behavior lives.
