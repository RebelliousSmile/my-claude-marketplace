...
1. Load the source document from `$ARGUMENTS`.
...
4. Cross-reference documentation: load ALL files declared in `bank.yml` sections `docs` and `rules-files`. Build a "constraints to distribute" list: mechanical rules, characters, factions, terminology.
