...
Create a new output-style file for a project from source documents (source mode), structured questionnaire (questionnaire mode), or a combination (hybrid mode).

## Inputs

- `<brief>` (required) — path to the brief directory (read-only)
...
Saved to: `<brief>/output-styles/<name>-<type>.md`
...
9. Write to `<brief>/output-styles/<name>-<type>.md`.
10. Report: file created, version, metrics (prose/list ratio, paragraphs per concept, example count).

## Test

After `analyze <brief>`, verify that `<brief>/output-styles/<name>-novel.md` (or the requested type) exists, contains a non-empty "Philosophie d'écriture" section and at least one real example.
