...
- **Sortie partagée** : ces sous-arbres sont l'artefact de référence du système pour la suite JDR solo d'obsidian (`solo-mc`, `pc`, `rpg`) — mécaniques, oracle, création de personnage. Pour le système Parallaxe : `JDR/parallaxe/canon/` (officiel) + `JDR/parallaxe/mj/` (house rules). Même logique de provenance que `lore-extract`.
