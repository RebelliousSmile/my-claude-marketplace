...
name: alias
model: sonnet
description: Fires a pre-crafted workflow prompt to chain aidd skills in one command, rewrite a prompt file in place (smarten), or critically audit code generated by another LLM without functional regressions (codex-vision). Use when you want to trigger plan→challenge or implement→review sequences, project snapshot with status context (previously), compress a .md prompt to its minimal form (smarten), reconcile a mockup vs implementation screenshot (mirror), or review AI-generated code against its functional contract. Do NOT use for single-skill tasks, custom workflows, or when you need direct control over each step.
---
