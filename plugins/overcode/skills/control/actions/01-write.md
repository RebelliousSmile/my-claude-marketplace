...
phase: <resolved phase> (declared | inferred | overridden | undetermined)
...
1-bis. Resolve the project **phase** per `@../references/phase-framework.md`, at the same rank as the documented strategy and the pivot: it is loaded context, not a decision input. It never changes the tier - see step 3 - and never populates `limit`.
