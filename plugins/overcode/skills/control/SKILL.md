...
- Every action resolves the target project's **phase** (`scaffolding` / `hardening` / `production` / `sustaining` / `undetermined`) per `references/phase-framework.md`, and reports it with its provenance. The phase **prioritises; it never classifies a tier** - same boundary as a pivot's *Risk signals*. It may move a gap to the top of a table or a low-value test to the top of a removal list, never change the tier proposed for either: tier authority stays with the loaded tier table. A test is refused on a tier criterion, never "because we are in production".
- Never delete a test file, apply a config fix, or write a proposed missing test, without a separate, explicit per-item confirmation from the user. No batch auto-apply.
...
- This skill never writes the target project's test strategy document. `aidd_docs/memory/testing.md` is owned by `aidd-context`'s project-memory skill, which generates and syncs it (the skill and action names have changed across `aidd-context` majors - refer to the document, never to a pinned action number); `control` only reads it. When that document is still the untouched generic template - test *types* or empty placeholders rather than decision criteria - treat it as absent for tier decisions, map unit and integration to `contract` and end-to-end to `e2e`, and say so in the output rather than pretending the project has a strategy.
