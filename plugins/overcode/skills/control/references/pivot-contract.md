...
- **Risk signals** (optional) - what is structurally high-consequence in this stack (money, auth, persistence, deletion, cross-cutting state) and what structurally does not deserve a test of its own (framework pass-through, generated glue). This field also carries the stack's **external boundaries** - the SDKs, tag containers, third-party scripts and outgoing API clients a project of this stack typically depends on, and how to detect them in a manifest. `control` owns the *external contract dependency* criterion generically (see `phase-framework.md`); it does not own the inventory, which is stack knowledge and would go stale in the skill at every new vendor. No new field is created for it: the authority split stated here applies unchanged. When the field is absent, `strengthen` falls back to reading the project's manifest for dependencies pointing at domains it does not control, and says the inventory is generic. Consumed by `strengthen` to weight its ranking. **Risk signals prioritise; they never classify a tier.** Tier authority stays with *Tier thresholds* and the generic decision framework - a signal may move a gap to the top of the table, never change the tier proposed for it. When absent, `strengthen` uses its own generic consequence weighting.
