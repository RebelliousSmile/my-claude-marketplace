...
**Generic core, then profile.** The primary contract is **domain-agnostic** (generic model `${CLAUDE_PLUGIN_ROOT}/references/domain-layout.md`): buckets have **free names** catalogued by `R/bank.yml`, feeders are the generic `writing:forge` (concept) + `obs:research` (data). The **JDR profile** (`${CLAUDE_PLUGIN_ROOT}/references/jdr-layout.md`) is one instance where the same catalogue happens to point into `_univers`/`_systeme` and the feeders are `ttrpg:lore-extract` / `ttrpg:rules-keeper`. **The assembly behaviour is identical across both — only the bucket names and feeder names change** (S13 pins exactly this).
