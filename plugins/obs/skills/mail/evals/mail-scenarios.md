...
### 2026-07-03 — run 1 (initial, dry-run, target=mail, fixture=scratchpad/fixtures/mail) — **19/19 PASS (0 N/A)**

Fixture state: `Thunderbird/` populated with 17 emails + `mail-config.yaml` (`merge_by_domain: true`, `preserve` domain `banque.ch`, `suppress` domain `klaviyo.com`, `prune` `jeveuxtravailler.com`/7d, contrary `exception` `alerte@banque.ch → suppress`, `phishing_brands: []`); judging "today" = **2026-07-02** (per suite), so `today−7 = 2026-06-25` and archive dir under test = `.archive/2026-07-02/`. Level-3 branches present (`Achats/MondialRelay/`, `Perso/Banque/Releves/`, `Factures/Orange/`); Orange facture carries `processed: true`. Second bare fixture `Thunderbird-fresh/` = 1 email (`spotify` reçu), **no** `mail-config.yaml`. Pre-flight data-precondition guard: **pass** (all guarded config keys present; brand list default; config absent in fresh fixture). Dry-run, READ-ONLY: nothing written to either fixture.

| #    | Resolved (intended writes / observable) | Verdict | Δ vs prior | Note (instruction cited) |
|------|------------------------------------------|---------|-----------|--------------------------|
| S1   | `content_action: delete`; archive `.archive/2026-07-02/email_2026-04-01_klaviyo_Soldes_to_moi.md` before removal | PASS | = (premier run) | suppress rung fires on `news@klaviyo.com` — SKILL § Pass A #1; 04-execute § delete #1–3. |
| S2   | keep `..._to_moi.md`, delete `..._to_moi_1.md` (`duplicate: true`, archived first) | PASS | = | exact-dup on `(subject_hash a1b2c3, from, date)`; date tie → first alphabetically (`.` < `_1`) — SKILL § Pass A #2; 02-analyze § Duplicate detection. |
| S3   | `content_action: delete`; archived first | PASS | = | prune match + `2026-01-05 < 2026-06-25` aged — SKILL § Pass A #3. |
| S3b  | `content_action: summarize`; **no** archive-for-delete | PASS | = | age gate bites: `2026-06-28 ≥ today−7` → prune does not delete; falls through — SKILL § Pass A #3 "`date < (today - days)`"; 02-analyze Pass A #2. |
| S4   | both `.com` files → one `merge_group`, `merge` + placement `none`; merged file in `Achats/MondialRelay/` | PASS | = | `merge_by_domain` normalizes both to `mondialrelay.com`; subj `Re:` stripped; both level-3 — SKILL § Thread detection / § Pass A #5 / Pass B; 04-execute § merge. |
| S5   | `content_action: intact` **and** `placement_action: classify` → `Perso/Banque/`; body not rewritten, file moved | PASS | = | preserve (domain `banque.ch`, no contrary exception for `conseiller@`) + Pass B at root — SKILL § Pass A #4 + Pass B, cumul example. |
| S6   | `content_action: flag-phishing`; move to `Publicités/Spam/Phishing/`, `flagged: phishing` + `processed: true` | PASS | = | display-name `PayPal` vs domain `paypal-secure.ru` mismatch overrides Pass A — SKILL § Phishing; 04-execute § flag-phishing. *(see friction: "match" predicate)* |
| S7   | no move/delete/rewrite and **no** `.archive/` write pre-`oui` | PASS | = | hard NO-GO — SKILL § Safety #1; 03-propose #3 (only `oui` → execute), Test "No batch executed without confirmation". |
| S8   | `.archive/2026-07-02/email_2026-04-01_klaviyo_...md` populated byte-identical **before** source deleted | PASS | = | archive-first — SKILL § Safety #2; 04-execute § "Before any irreversible action" + § delete #1–3. |
| S9   | main chat carries only paths + structured decisions; zero body text | PASS | = | sub-agent delegation, "without email content" — SKILL § Sub-agent principle; 01-scan step 2 / 02-analyze Return, Tests "No email content in main chat". |
| S10  | `intact` + placement `none`; **no** `processed: true` key written; stays in scope | PASS | = | hard NO-GO on tag — SKILL § `processed` tag "intact files are not marked"; 04-execute § intact "Do not add `processed: true`". |
| S11  | pipeline halts before `analyze`; template shown + confirmation asked; file created only on confirm | PASS | = | config-absent boundary — 01-scan #4 "If absent: display… ask for confirmation before continuing"; SKILL § template. |
| S12  | default `file_list` omits Orange facture; `--reprocess` includes it | PASS | = | `processed: true` exclusion — SKILL § `processed` tag; 01-scan #2 + Test. |
| S13  | `.fr` file **absent** from S4 `merge_group`; `content_action: summarize` (transactionnel) | PASS | = | country TLD kept: `mondialrelay.fr ≠ mondialrelay.com` — SKILL § Thread detection; 02-analyze #4. |
| S14  | `content_action: summarize` (transactionnel); **not** flagged, no `flagged:` key, not moved to Phishing | PASS | = | phishing discriminance: `amazon.fr` matches brand → no flag — SKILL § Phishing "domain does not match"; 02-analyze § Phishing. *(see friction)* |
| S15  | `summarize`, `summary_type: newsletter`; body kept to date · titre · liens d'update | PASS | = | Patreon/update taxonomy — SKILL § Taxonomy › Newsletter/update; 04-execute § summarize `newsletter`. |
| S16  | `summarize`, `summary_type: notification`; body kept to service · date · action requise | PASS | = | login/alert taxonomy; `ovh` not in brand list → no flag — SKILL § Taxonomy › Notification/alerte; 04-execute § summarize `notification`. |
| S17  | `summarize`, `summary_type: promotionnel`; body kept to offre · date d'expiration | PASS | = | promo taxonomy, not in `suppress` — SKILL § Taxonomy › Promotionnel; 04-execute § summarize `promotionnel`. |
| S18  | `content_action: delete` (**not** `intact`); archived first | PASS | = | contrary exception blocks preserve → suppress → delete — SKILL § Pass A #4 "AND no contrary exception" + #1; 02-analyze Pass A #3. *(see friction: exception-action wiring)* |

**Frictions / gaps:**
- **Phishing "domain match" predicate is under-specified (affects S6 + S14).** SKILL § Phishing says flag when the display-name brand is present "but the address domain does not match" — without defining *match*. A naive substring reading makes *both* `paypal-secure.ru` and `amazon.fr` "contain" their brand → neither flags (S6 would FAIL). The only reading consistent with both rows is registrable-label equality (`paypal-secure` ≠ `paypal` → flag; `amazon` == `amazon` → no flag). Both PASS on that faithful reading, but the predicate should be spelled out (e.g. "compare the registrable domain's principal label to the brand") to keep the two rows genuinely discriminant.
- **Exception→action wiring is implicit (S18).** SKILL § Pass A #4's "AND no contrary exception" explicitly blocks `intact` (so the hard FAIL trigger — resolving `intact` — cannot fire), but no rung mechanically states that an `exceptions` entry with `action: suppress` then forces the file down the suppress rung. A literal ladder walk after preserve is blocked (address not in `suppress.senders`, no dup/prune/thread) could fall through to `summarize` instead of `delete`. Faithful reading applies the exception's declared action → `delete`, hence PASS, but the criterion's FAIL trigger ("if resolved intact") is narrower than its PASS requirement ("content_action: delete"), so a `summarize` fall-through would be an un-caught soft miss. Recommend an explicit rule: "an `exceptions` entry re-routes the file to its `action`'s rung."
- No NO-GO scenario is data-blocked; no data-limit notes this run.

**Tally:** 19/19 PASS (0 N/A) — green baseline established. Run 1, no prior on this suite → Δ = premier run for every row. Suite pins current specified behaviour; a future edit that breaks a ladder rung, the archive-first guard, the confidentiality delegation, the `processed`/`intact` discipline, or the phishing discriminance will surface as PASS→FAIL against this baseline.
