...
### 2026-07-03 — run 2 (post-fix, dry-run, target=tree, fixture=documents-perso-atelier) — **20/20 PASS (0 N/A)**

Re-run after the suite gained **S18** (link integrity on move) and the SKILL gained two Transversal rules: **"Target scope is strict, not anchor-wide"** (check/fix bound their report/plan to `<target>` only) and **"Link integrity on move"** (rewrite inbound `[[…]]`, update/co-move embeds `![[…]]` + relative assets, verify no dangling ref; applies to `fix`/`sort`/`judge`). Fixture `documents-perso-atelier` re-built at `scratchpad/fixtures/tree-postfix` with the **S18 linked triplet**: `zombiology/note-a.md` (embeds `![[diagram.png]]`, targeted by `[[note-a]]`) + `note-b.md` (holds the incoming `[[note-a]]`) + `_assets/diagram.png`. All run-1 preconditions preserved: STALE `_tree/cache.json` (`scanned_at: 2026-06-28`, `scene-finale/` snapshot lists only `notes.md`; `faction-lore.md` = post-scan delta); curated `zombiology/R/bank.yml` (`patient-zero` summary) + curated `destinations.txt`; both `.env` (`PLACEHOLDER=x`, never opened), `.git/`, `mood-board.png`, collision `scene-brouillon.md`, `Scene Brouillon.md` (I3), `_lore.md` (I2), `2026/6/` (I4), unprefixed `Bank/example/2026/05/drafts/` (I1), `notes.md` (prose + 1 code fence + 1 URL). Pre-flight checker: **n/a** (no machine data-integrity checker; `scenarios.json` untouched). Judge ran READ-ONLY — nothing written to the fixture; `.env` files never read.

| # | Action | Intended writes (paths + scope) | Verdict | Δ vs run 1 | Note (instruction cited + fixture element) |
|---|--------|--------------------------------|---------|---|---|
| S1 | `index Perso/` | `_tree/cache.json` (overwrite) + `zombiology/R/bank.yml` (merge); no user-content move | PASS | = | SKILL › Transversal + 01-index steps 6–7; index is anchor-wide by design (no strict-scope bound). |
| S2 | `check zombiology` | stale-cache refresh only | PASS | = | 02-check steps 3–4 report-only. In-scope hard anomalies `Scene Brouillon.md`→I3, `_lore.md`→I2, `2026/6`→I4 + `faction-lore.md`→soft drift; hard/soft split explicit. **`drafts/` (I1) is out of `<target>`** under new strict-scope → correctly omitted (see frictions). |
| S3 | `fix zombiology` | dry-run plan only pre-confirm (`Scene Brouillon.md→scene-brouillon.md` ON; `faction-lore.md→_univers/` OFF) | PASS | = | 03-fix steps 3–5: invariant-ON/drift-OFF, confirm before disk, no delete. Plan bounded to `<target>` — `drafts/→_drafts` (in `Bank/example`) correctly excluded under strict-scope Inputs. |
| S4a | `sort zombiology-session-notes.md` | none pre-confirm | PASS | = | 04-sort step 3 "confident single match → propose directly", names `RPG/zombiology`. |
| S4b | `sort releve-campagne.md` | none (escalates) | PASS | = | 04-sort step 3 "ambiguous → ask user to choose"; ≥2 ranked candidates, zero move. |
| S5 | `judge` supprimer | move → `R/_trash/<file>`, no `rm` | PASS | = | 05-judge Phase 3 + Rules "Never delete"; `_trash/` present. |
| S6 | `judge`/`index` @ `.env` | none; `scene-finale/.env`→credentials signal (unread); `_code/.env`→silent skip (unread) | PASS | = | SKILL › Credentials + `_code/` exception; 05-judge Phase 0. |
...
| S9 | `index` re-derive `bank.yml` | merge; curated `patient-zero` summary retained verbatim | PASS | = | SKILL › "merge, not clobber"; 01-index step 7. |
| S10 | `judge` @ `mood-board.png` | none; media excluded from queue | PASS | = | SKILL › "Media files — skip, never judge or read"; 05-judge Scope. |
| S11 | `index Downloads/` | no `_tree/` without consent | PASS | = | SKILL › "no anchor → report + offer managed root"; 01-index step 1. |
| S12 | `check Perso/` (stale) | cache refresh (re-scan disk) then report | PASS | = | 02-check step 2 "disk is source of truth"; catches `faction-lore.md` delta. |
| S13 | `judge` résumer `notes.md` | in-place rewrite AFTER showing content; fence + URL byte-identical; `summarized: true` | PASS | = | 05-judge Phase 3 Résumer steps 2–5 + Rules "Show before write". |
| S14 | `destinations --out ../destinations.txt` | no write until diff shown + approval; curated file unchanged meanwhile | PASS | = | 06-destinations step 8 + Rules "never overwrite curated file without diff + confirmation". |
| S15 | `check zombiology` (`_lore.md`) | cache refresh only | PASS | = | 02-check step 3 "I2 prefixed content in working dir"; suggest `_lore.md → lore.md`. In-scope. |
| S16 | `check zombiology` (`2026/6/`) | cache refresh only | PASS | = | 02-check step 3 "I4 malformed year/month"; suggest `2026/06`. In-scope. |
| S17 | `judge` CWD=`Downloads/` | none — STOP, 0 nodes enqueued, 0 files read | PASS | = | 05-judge Inputs "no anchor → STOP: 'Aucun domaine R trouvé…'". |
| S18 | `fix`/`sort` relocate `note-a.md` (incoming `[[note-a]]` + embed `![[diagram.png]]`) | dry-run plan → after confirm: move `note-a.md` + rewrite `[[note-a]]` in `note-b.md` + co-move `_assets/diagram.png` (or rewrite embed path) + verify no dangling ref; no delete/overwrite | PASS | **new** | SKILL › Transversal "Link integrity on move" (rewrite `[[…]]`, update/co-move `![[…]]`+relative assets, verify no dangling ref; applies to `fix`/`sort`). All three required sub-behaviours mandated; gated on dry-run + confirm. **Gap:** rule lives only in SKILL Transversal — not operationalized in 03-fix/04-sort process steps (see frictions). |

**Frictions / gaps:**
- **S18 rule not operationalized in the action files (spec-hardening, non-blocking).** "Link integrity on move" is mandated in SKILL Transversal and binds all actions, so a spec-faithful target PASSes. But 03-fix (step 3 builds a plan of `mv`/rename `from → to` rows only; step 5 safety gate covers collisions/no-delete) and 04-sort (step 5 move) contain **no explicit step** to scan for inbound wikilinks, rewrite them, co-move embedded assets, or run a post-move no-dangling-reference check. An implementer following only the action steps could complete the move and leave `[[note-a]]`/`![[diagram.png]]` broken. Recommend adding a "link-integrity pass" sub-step to 03-fix step 5 and 04-sort step 5 (and 05-judge Phase 3 Avancer) so the decisive S18 observable is enforced at the action layer, not only the SKILL layer.
- **S2/S3 `drafts/` example now confirmed out-of-scope (Δ from run 1's "mild false-good risk" → concrete scenario defect).** The new **"Target scope is strict"** rule (SKILL Transversal + 02-check/03-fix Inputs) means a `check`/`fix Perso/RPG/zombiology` run reports/plans **only** items under that subtree. `drafts/` (I1) lives in `Perso/Bank/example` — a sibling domain **outside** the target — so a spec-faithful run correctly **omits** it. The scenarios' pass criteria still name "the two slug/prefix issues" incl. `drafts/`, but the core behaviour under test (report-only, hard/soft split explicit + correct) holds robustly on the **in-scope** hard anomalies (`Scene Brouillon.md` I3, `_lore.md` I2, `2026/6` I4) + soft drift (`faction-lore.md`) → **PASS stands, verdict unchanged (Δ =)**. Fix the scenarios: retarget S2/S3 to `Perso/`, or swap `drafts/` for an in-scope I1 example (zombiology has no unprefixed working dir today).
- **Carried from run 1 (unchanged):** absent `references/*` (decisive observables restated inline, so no FAIL, but `bank.yml`/`destinations.txt` exact formats + sort bucket taxonomy unverifiable); S6 two `.env` under different anchors only contrastable across two runs; S12 staleness trigger under-specified (no mtime-compare mechanism stated).
- No over-broad instruction forces a clobber/`rm`/credential-read/standalone-dotfile-move/media-read/dangling-link. `.env` files never opened; nothing written to the fixture.

**Tally:** 20/20 PASS (0 N/A), 0 FAIL. Δ vs run 1: **+1 scenario (S18, new, PASS)**; all 19 prior verdicts hold (`=`) — **no regression** from the two new SKILL rules. Suite remains green. Two frictions are spec-hardening (operationalize link-integrity in the action steps; retarget S2/S3 for strict scope), not behavioural misses.
