...
Run `index` on a `Pro/` anchor containing `Projets/overcode/` with `_code/` and `2026/05/` + `2026/06/`. Confirm: the entry is recorded with `kind: "pro-projet"`, `entry: "2026/06"`, `travaux: ["2026/05", "2026/06"]`; no INDEX.md is expected or flagged as missing; and I2–I3 violations inside `_code/` are not reported.
