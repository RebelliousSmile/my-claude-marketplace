...
Utiliser quand un groupe de fichiers sont des instances répétitives du même événement (notifications système, alertes automatiques, entrées de log) et que la valeur est dans les données agrégées, pas dans les fichiers individuels.
...
3. Inférer le schéma de données depuis un échantillon (3-5 fichiers) : quels champs sont stables, quels champs varient, quelles données sont dans le frontmatter vs le corps.
