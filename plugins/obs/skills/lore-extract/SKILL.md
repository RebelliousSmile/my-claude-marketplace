...
- **Shared tree** : these `canon/` + `mj/` subtrees (same file names, "one info in a single file" rule) are **shared with `obs:rpg`** for JDR universes. A universe lives in `<univers-root>` = `R/_univers/<univers>/` within the self-contained game domain `R` (discovered locally — see `${CLAUDE_PLUGIN_ROOT}/references/jdr-layout.md`). `rpg` writes GM content into the same `mj/`. Keep the structure for interoperability.
- **Regenerable sources** — A universe's `sources/` (`<univers-root>/sources/`, this skill's raw inputs) are **large and regenerable** from the official PDFs; if `R` is versioned, it is the natural candidate to gitignore. The `canon/` + `mj/` output, by contrast, is durable authoritative content — consumers (`write`, `review`, `obs:rpg`) stay operational without a re-run. To **regenerate or enrich** the lore without the `sources/`, first re-extract via `extract-pdf` (commercial PDF required), then re-run `lore-extract`.
