...
> **Le guidage propre à un jeu** (règles maison, pièges de session, adjudication des moves/réactions MC) vit **dans le domaine de jeu**, sous `R/_systeme/{canon,mj}/`, **pas dans la skill** : T0 résout `R`, T6 lit les règles automatiquement. La skill reste agnostique du jeu. Convention d'arborescence : voir `${CLAUDE_PLUGIN_ROOT}/references/jdr-layout.md`.
...
- **T6** — System mechanics (oracle, dice, character creation, scene rules) come from the active system's **rules-keeper-optimized rules**, produced by `writing:rules-keeper`, split by provenance: `canon/` (official ruleset) + `mj/` (the GM's house rules, which explicitly override canon where declared). Effective rules = canon + declared house rules; consult both, never invent mechanics. **Generic subsystems** (e.g. Parallaxe, Cinério, Muses et Oracles) are live-play tools layered on the game system (not games themselves); **`solo-mc` is their sole consumer**, at `R/_subsystems/<name>/{canon,mj}/` — produced by `writing:rules-keeper`. The game system's own rules at `R/_systeme/{canon,mj}/` are shared with `pc` and `rpg`; subsystems are not.
- **T7 — Régénération des règles canon** — `R/_systeme/canon/` (sortie de `rules-keeper`) et tous les `sources/` (entrées brutes) sont **dérivés de matériel commercial et régénérables** depuis les PDF ; si `R` est versionné, ce sont les candidats naturels à gitignorer (`**/sources/`) — un choix local au dépôt qui héberge `R`, pas une dépendance de la skill. S'ils sont absents (p. ex. domaine fraîchement récupéré sans ces dossiers), régénérer `R/_systeme/canon/` — relancer `extract-pdf` (PDF commercial requis) puis `rules-keeper` — **avant toute session**, sinon mécaniques, oracle de base et création de personnage sont indisponibles. Durables (à conserver) : les sous-systèmes `R/_subsystems/<nom>/{canon,mj}/`, le lore d'univers `R/_univers/<univers>/{canon,mj}/` et les règles maison `R/_systeme/mj/` (dont `solo.md`, les house rules de jeu solo établies en partie).
...
- **T13 — Grille décisionnelle (anti-linéarité)** — Appliquer à **chaque action de PJ et point de décision** pendant le jeu, **quel que soit le point d'entrée** : en session active, **toute** réponse passe par cette grille (Étape 1 = l'action est-elle incertaine ?) **avant** toute narration d'issue — il n'existe aucun chemin de narration libre qui la contourne (le déclenchement n'est donc pas suspendu au seul `scene` : tout tour RP du joueur y est soumis). Étapes :
  - **Étape 1 — L'action du PJ est-elle incertaine ? (principe agnostique, AVANT toute narration d'issue)** : **dès qu'une action de PJ a une issue incertaine et à enjeu, on statue avec les règles du système actif** (`R/_systeme/canon/`, + `_subsystems/`) — jamais en narrant librement. La *forme* de résolution dépend du système : **déclencheur textuel** d'une move (PbtA → `2d6 + carac`, paliers 10+/7-9/6-), **jet sous seuil** (d100 ≤ Compétence% + Carac%), **vs DC** (d20)… → **nommer ce qu'on résout** (move ou test de compétence), **demander le jet prescrit par le canon** (le MC ne lance jamais, le joueur lance / router vers `roll`), **appliquer le modèle d'issue du système, tel qu'écrit** (paliers, réussite/échec + critiques en X0, marge vs DC…), et sur échec appliquer ce que **ce** système prévoit (PX, niveau de stress, complication, réaction MC) — sans importer la mécanique d'un autre jeu. Faire remonter toute branche propre (un 7-9 → démon intérieur, un critique, un coût de ressource). **Ne jamais narrer librement une issue incertaine.**
  ...
  - **Étape 3 — Routage par enjeu** :
    ...
    - **Nouveau lieu ou PNJ nommé** → le nommer, écrire une description d'une ligne, promouvoir dans `R/_campagnes/<campagne>/mj/` (ou `R/_univers/<univers>/mj/` si portée mondiale ; règle de conduite solo → `R/_systeme/mj/solo.md`). Routage complet : voir « Routage des faits de fiction » dans `${CLAUDE_PLUGIN_ROOT}/references/jdr-layout.md`.
    - **Décision à enjeu** (issue incertaine, conséquence joueur, embranchement narratif) → si **une move du système couvre l'enjeu**, elle relève de l'**Étape 1** (jet + palier), **pas** de l'oracle. Sinon (aucune move applicable), résoudre via l'oracle (muses-et-oracles pour le hasard ; parallaxe pour la décision) ; résultat relié à un test, une manœuvre ou un élément de règle ; ne JAMAIS narrer l'issue librement.
...
- **T14 — Substitution de compagnon (équipe séparée)** — Décision MC, pas une commande joueur. Signaux déclencheurs : une scène s'ouvre sur un lieu où seul le compagnon est présent ; la fiction crée un embranchement où deux personnages doivent agir simultanément en des lieux différents ; la dernière action du joueur envoie le PJ d'un côté et le compagnon ailleurs.
  ...
  - **Dégradation gracieuse** : si la fiche compagnon est absente → `[HRP] Fiche compagnon pour <nom> introuvable à R/_pjs/<pj>/compagnons/<slug>.md. Lance /obs:pc companion create <nom> d'abord.`
...
- **Narrer une issue incertaine sans la résoudre** — dès qu'une action de PJ a une issue incertaine et à enjeu, statuer **avec les règles** du système actif (move déclenchée, test de compétence sous seuil, jet vs DC… selon le système), jamais en narrant librement. À l'inverse, ne **pas** forcer un jet quand l'issue est certaine/sans enjeu. Le déclencheur universel est l'**incertitude**, pas un mot-clé imprimé : sur un système sans déclencheur textuel, « pas de trigger » ne veut **pas** dire « pas de jet ».
- **Chronologie / continuité** — recouper les fichiers avant d'introduire un élément nouveau.
- **Datation / numérotation de session** — vérifier la date système ; numéroter d'après les fichiers `session-*.md` sous `R/<AAAA>/<MM>/<campagne>/` (balayer **tous** les dossiers année/mois ; la vérité est dans le système de fichiers, **pas** dans `config.yaml › session_courante`/`last_played`, qui peuvent pointer une archive).
