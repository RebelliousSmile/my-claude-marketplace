...
2. Load existing universe docs for comparison from `<univers-root>/canon/`: `UNIVERS.md`, `terminologie.md`, and any other canon files declared in `bank.yml`.
...
After `research "topic"`, verify that `<projet-root>/research/<slug>-<date>.md` exists and contains at least 3 distinct source references, a contradictions section (even if empty), and a comparison with existing docs. On confirmation, verify that validated findings are written to `<univers-root>/canon/`.
