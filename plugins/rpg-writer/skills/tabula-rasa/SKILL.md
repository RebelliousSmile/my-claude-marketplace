...
Destructive project reset. Removes generated files (chapters, TOC entries, WIP artifacts) while preserving universe documentation (`<univers>/`), output-styles, personas, and `bank.yml`. Requires explicit user confirmation before any deletion. Optionally creates a backup archive before resetting.
...
- NEVER delete `bank.yml`, `<univers-root>/`, `<univers-root>/.output-styles/`, `<univers-root>/.templates/`.
