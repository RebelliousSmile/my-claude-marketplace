...
name: rules-keeper
model: sonnet
description: Restructures any game rules file into an LLM-optimized format (CHEATSHEET + LEXICON + PATTERNS + ENTITY TEMPLATES + FULL REFERENCE) ready for use by `write` and `review` skills — and by the solo-RPG suite (`obsidian:solo-mc` for live play, `obsidian:pc` and `obsidian:rpg`) as the shared system-rules reference, separating the official ruleset (canon/) from the GM's house rules (mj/, `--homemade`). Use when integrating a new game system, when rules files are verbose or poorly structured, or when merging a supplement into an existing rules file. Do NOT use for extracting lore or universe documentation — use `lore-extract` instead; do NOT use to write new rules from scratch — use `write` instead.
---
...
- **Sortie partagée** : ces sous-arbres sont l'artefact de référence des règles pour la suite JDR solo d'obsidian. Le coffre JDR est rangé **par jeu** (`<jeu>` = premier segment sous la racine). Deux cas de provenance distincts :
  - **Système de jeu** → `<vault>/<jeu>/_systeme/{canon,mj}/`. Référence mécanique **partagée par `obsidian:solo-mc`, `obsidian:pc` et `obsidian:rpg`** (mécaniques, oracle de base, création de personnage). Le fichier `mj/solo.md` y est écrit par `obsidian:solo-mc` en jeu (house rules de jeu solo) — voir `@setup/references/vault-layout.md`.
  - **Sous-système générique** (module réutilisable greffé sur un système — ex. **Parallaxe**, **Cinério**, **Muses et Oracles**) → `<vault>/<jeu>/_subsystems/<nom>/{canon,mj}/`, avec repli sur les sous-systèmes **partagés** entre jeux dans `<vault>/_subsystems/<nom>/{canon,mj}/`. `rules-keeper` les **produit**, mais ils sont **consommés par `obsidian:solo-mc` uniquement** (outils de jeu en direct) — ni `pc` ni `rpg` ne les référencent.

  Même logique de provenance que `lore-extract`.
- **Convention de versioning (`tnn-jdr`)** — Dans le dépôt `tnn-jdr`, l'**output** de `rules-keeper` pour le système de jeu (`<vault>/<jeu>/_systeme/canon/`) **et** son **input** (les sources de règles brutes, `<vault>/<jeu>/_systeme/sources/<source>/`) sont **gitignored** (`**/_systeme/canon/`, `**/sources/`). Conséquence : après un clone sur une nouvelle machine, **ni** le canon régénéré **ni** les sources brutes n'existent — reconstruire le canon impose un **setup from scratch** complet depuis le PDF commercial (voir `@02-restructure-all.md › Setup from scratch`). Non couverts par le glob `**/_systeme/canon/`, donc versionnés : les règles maison `_systeme/mj/` et les sous-systèmes `_subsystems/<nom>/canon/`.
