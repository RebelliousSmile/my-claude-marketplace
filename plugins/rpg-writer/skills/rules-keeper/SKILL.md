...
- **Sortie partagée** : ces sous-arbres sont l'artefact de référence des règles pour la suite JDR solo d'obsidian (`solo-mc`, `pc`, `rpg`) — mécaniques, oracle, création de personnage. Vaut aussi bien pour un système de jeu que pour un **sous-système générique** (module de règles réutilisable employé par un jeu). Le coffre JDR est rangé **par jeu** (`<jeu>` = premier segment sous la racine) : le **système de jeu** lui-même va dans `JDR/<jeu>/systeme/{canon,mj}/` ; un **sous-système** local au jeu dans `JDR/<jeu>/subsystems/<nom>/{canon,mj}/`, avec repli sur les sous-systèmes **partagés** entre jeux dans `JDR/subsystems/<nom>/{canon,mj}/`. Ex. de sous-systèmes : **Parallaxe**, **Cinério**, **Muses et Oracles**. Même logique de provenance que `lore-extract`.
