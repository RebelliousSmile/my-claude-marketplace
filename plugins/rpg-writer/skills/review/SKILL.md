...
- **Setup après clone (`tnn-jdr`)** — La terminologie consommée (`<univers-root>/canon/terminologie.md`) est **versionnée** : elle survit au clone, donc `review` reste opérationnel. Contexte seulement : si la chaîne amont doit être régénérée, les `sources/` et `systeme/canon/` sont gitignored et nécessitent un re-run `extract-pdf` → `rules-keeper` / `lore-extract`.
...
- `<univers-root>/canon/terminologie.md` — canonical terminology.
