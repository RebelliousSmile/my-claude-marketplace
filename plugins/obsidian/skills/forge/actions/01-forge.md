...
> If the project belongs to a JDR domain, universe docs are resolved locally: `R/_savoir/univers/<univers>/{canon,mj}/` where `R` is discovered by walking up to the `_savoir/` marker. See `${CLAUDE_PLUGIN_ROOT}/references/jdr-layout.md`.
...
1. Locate `<projet>/_brief/overview.md`. If `_brief/` is absent, create it (or suggest `obsidian:brief assemble <projet>` to set up the working dir first). Read the project `type` from the overview frontmatter (default: "scenario"); ask if unset.
2. If the project belongs to a JDR domain, load the relevant universe docs for consistency: resolve `R` locally (walk up to `_savoir/`), then read `R/_savoir/univers/<univers>/canon/` (official lore) and `.../mj/` (MJ additions). For a non-JDR project, skip this step.
...
9. **Exit check**: all required overview elements present AND the last 2–3 iterations produced only minor adjustments? → Present the completion summary and suggest the next step: `obsidian:brief assemble <projet>` to consolidate the brief, then `writing:toc`.
