...
- Always update the overview **only in `<projet>/_brief/overview.md`** (working dir `_brief/`, content not prefixed). `obsidian:brief` later consolidates this overview (with lore/rules/data) into the self-contained `_brief/summary.md`.
...
- `${CLAUDE_PLUGIN_ROOT}/references/jdr-layout.md` — local resolution of a JDR domain `R` and its universe docs (`R/_savoir/univers/<univers>/`), for consistency checks when the project belongs to a game.
