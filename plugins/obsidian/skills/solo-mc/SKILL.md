...
- **T6** — System mechanics (oracle, dice, character creation, scene rules) come from the active system's **rules-keeper-optimized rules**, produced by `writing:rules-keeper`, split by provenance: `canon/` (official ruleset) + `mj/` (the GM's house rules, which explicitly override canon where declared). Effective rules = canon + declared house rules; consult both, never invent mechanics. **Parallaxe** is a **subsystem** layered on the game system (not a game itself); the obsidian suite (`pc`, `rpg`) uses it at `JDR/parallaxe/canon/` + `JDR/parallaxe/mj/` — the **same artifact** these skills reference, alongside the game system's own rules.
