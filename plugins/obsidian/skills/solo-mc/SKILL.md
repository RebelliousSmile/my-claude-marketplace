...
- **T0 — Racine par jeu** — Le coffre `C:/Users/fxgui/Public/Notes/Perso/JDR/` est rangé **par jeu** sous `JDR/<jeu>/`. `<jeu>` est le premier segment sous la racine, déduit du répertoire courant ou de la campagne active. Ressources : campagnes `JDR/<jeu>/campagnes/<campagne>/`, personnages canoniques `JDR/<jeu>/pjs/<pj>/`, système du jeu `JDR/<jeu>/systeme/{canon,mj}/`, sous-systèmes locaux `JDR/<jeu>/subsystems/<nom>/{canon,mj}/` (repli partagé `JDR/subsystems/<nom>/`). **Univers** : un setting est un univers du jeu, à `JDR/<jeu>/univers/<univers>/.docs/{canon,mj}/` (un jeu peut en avoir plusieurs, chacun partagé par son groupe de campagnes) ; la campagne déclare le sien via `config.yaml › univers: <slug>`.
...
- **T6** — System mechanics (oracle, dice, character creation, scene rules) come from the active system's **rules-keeper-optimized rules**, produced by `writing:rules-keeper`, split by provenance: `canon/` (official ruleset) + `mj/` (the GM's house rules, which explicitly override canon where declared). Effective rules = canon + declared house rules; consult both, never invent mechanics. **Generic subsystems** (e.g. Parallaxe, Cinério, Muses et Oracles) are layered on the game system (not games themselves); the obsidian suite (`pc`, `rpg`) uses them at `JDR/<jeu>/subsystems/<name>/{canon,mj}/` (game-local) with fallback to the shared `JDR/subsystems/<name>/{canon,mj}/` — the **same artifacts** these skills reference, alongside the game system's own rules at `JDR/<jeu>/systeme/{canon,mj}/`.
