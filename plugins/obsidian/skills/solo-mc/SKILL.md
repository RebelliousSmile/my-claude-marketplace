...
- **T10 — Journalisation narrative continue (obligatoire)** — Pendant une session active (`play` / `play-resume`), **avant de répondre**, archiver l'échange précédent (message RP du joueur + réponse MJ précédente) dans le fichier de log de session (`R/_campagnes/<campagne>/<YYYY>/<MM>/<campagne>-session-<YYYY-MM-DD>-<N>.md`, défini par `play`) : lire le log, y concaténer l'échange, écrire, **puis** répondre. Ce log narratif est **distinct** de `R/_campagnes/<campagne>/.session-state.yaml` (état mécanique, écrit au `play-end`). Les `[HRP]` sont loggés mais identifiés.
...
- **Datation / numérotation de session** — vérifier la date système ; numéroter d'après les fichiers `<campagne>-session-*.md` sous `R/_campagnes/<campagne>/<AAAA>/<MM>/` (la vérité est dans le système de fichiers, **pas** dans `config.yaml › session_courante`/`last_played`, qui peuvent pointer une archive).
