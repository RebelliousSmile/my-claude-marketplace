...
Les fiches wrapper vivent sous `_pjs/<pj>/compagnons/` ; le **roster PJ-level** (`_roster.yaml`) les liste, et le `config.yaml` d'une campagne (si elle existe) ne fait que **référencer** ce roster (comme l'instance de campagne référence le PJ canonique). Lus par `obsidian:solo-mc` en jeu pour la substitution.
