...
Partir du répertoire de référence (`<project_dir>` ou CWD) et remonter les parents jusqu'au premier dossier contenant le marqueur `_savoir/` :

```bash
python -c "
from pathlib import Path
ref = Path('<project_dir>').resolve()
R = next((p for p in [ref, *ref.parents] if (p / '_savoir').is_dir()), None)
print('[OK] R =', R) if R else print('[MISSING] _savoir/ introuvable en remontant')
"
```

If missing → STOP: "Cible hors d'un domaine JDR initialisé (pas de `_savoir/` en remontant). Initialiser `R` d'abord."
...
- `<univers>` = argument reçu (slug `kebab-case`) ; si absent, demander à l'utilisateur. Cible : `<univers-root> = R/_savoir/univers/<univers>/`.
