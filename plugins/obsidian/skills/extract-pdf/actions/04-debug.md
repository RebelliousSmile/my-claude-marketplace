...
2. Découvrir `R` (remonter du répertoire de référence jusqu'au marqueur `_savoir/`) ; lire `<univers>` dans `progress.md#Univers` → construire `<univers-root>` = `R/_savoir/univers/<univers>/` et `<systeme-root>` = `R/_savoir/systeme/`. Tout vit sous le même dépôt `R` (un seul `git -C "<R>"`).
