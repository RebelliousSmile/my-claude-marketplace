...
- **Boîte à outils de craft (system-agnostic)** : consulter `references/methodes-mj.md` — techniques de maîtrise transversales (cartographier les relations, PNJ miroir, cadeaux empoisonnés, objectifs contradictoires, scènes qui comptent, garde-fous…), sourcées des recueils *Sortir de l'auberge* (Lapin Marteau). Elles portent sur le **craft** et s'appliquent **quel que soit le système**. **Ne jamais y mêler de mécanique propre à un jeu** : un framework comme l'agenda/principes/manœuvres MC est spécifique à **PbtA** (Monsterhearts) et reste dans `systeme/canon` de ce jeu — il ne s'applique pas à un jeu sim comme Zombiology. Pour toute mécanique, déférer à `systeme/{canon,mj}`.
