...
- **Racine par jeu** : le coffre `C:/Users/fxgui/Public/Notes/Perso/RPG/` est rangé **par jeu** sous `RPG/<jeu>/` (`<jeu>` = premier segment sous la racine, déduit du contexte de campagne actif via `.current-session`, pas d'un répertoire deviné). Une campagne vit dans `RPG/<jeu>/_campagnes/<campagne>/` (même convention que `solo-mc`), avec `config.yaml`, `pj/`.
- **`<univers-root>` (résolution de l'univers)** : un **setting est un univers du jeu**, rangé au niveau jeu `RPG/<jeu>/_univers/<univers>/` — même s'il est vaste (gamme étendue, plusieurs éditions, ex. 7ème Mer / Théah pour le jeu Noblesse Oblige). Un jeu peut avoir **plusieurs univers** ; chaque univers est partagé par le **groupe de campagnes** qui s'y déroulent. La campagne déclare le sien (`config.yaml › univers: <slug>`). Dans les règles ci-dessous, `<univers-root>` = `RPG/<jeu>/_univers/<univers>/`.
- **Deux niveaux à distinguer** :
  - **Données univers (durables, transverses aux campagnes)** — terminologie, factions, personnages, lieux/géographie, histoire. Elles vivent dans l'**arborescence partagée avec `lore-extract`** : `<univers-root>/` (= `RPG/<jeu>/_univers/<univers>/`), **scindée par provenance en deux sous-arbres thématiques identiques** :
  ...
  - **Prep de campagne (spécifique à une partie)** — scénarios, prep de session, fronts/horloges actifs, accroches PJ. Elle vit **dans le dossier de campagne** : `RPG/<jeu>/_campagnes/<campagne>/{scenarios,prep}/` + l'état des fronts.
...
- **PJ canonique vs instance de campagne** : le personnage durable vit dans `RPG/<jeu>/_pjs/<pj>/` (skill `pc`, source de l'`intention.md`) ; l'instance jouée d'une campagne vit dans `RPG/<jeu>/_campagnes/<campagne>/pj/` (`solo-mc`) et **référence** le PJ canonique. `rpg` s'ancre toujours sur l'`intention.md` canonique de `pc` ; si seul un PJ de campagne existe, le signaler et proposer de le rattacher à un PJ `pc`.
- La campagne déclare son univers (`config.yaml › univers: <slug>` → `RPG/<jeu>/_univers/<univers>/` ; un jeu peut en avoir plusieurs) ; à défaut, demander quel univers et le lister sous `RPG/<jeu>/_univers/`.
- Demander le nom de la campagne s'il n'est pas dans `$ARGUMENTS` ; lister les campagnes existantes sous `RPG/<jeu>/_campagnes/` (dossiers contenant un `config.yaml`).
...
- **Système de jeu** : pour toute mécanique (récompenses, tags PNJ, défis), `rpg` consulte les règles du système de jeu au format rules-keeper (`writing:rules-keeper`), scindées canon/mj, sous `RPG/<jeu>/_systeme/{canon,mj}/`. Règles effectives = canon + house rules déclarées. **Ne jamais inventer de mécanique.** Les **sous-systèmes génériques** (Parallaxe, Cinério, Muses et Oracles) sont des **outils de jeu en direct** consommés par `solo-mc` uniquement — hors du ressort de `rpg`.
...
- `RPG/<jeu>/_campagnes/<campagne>/config.yaml` — paramètres de campagne (créés par `solo-mc setup`)
- `RPG/<jeu>/_univers/<univers>/canon/` — lore officiel (lecture seule pour `rpg` ; écrit par `lore-extract`)
- `RPG/<jeu>/_univers/<univers>/mj/` — contenu créé par le MJ (écrit par `rpg`) ; même arborescence thématique que `canon/`
- `RPG/<jeu>/_pjs/<pj>/intention.md` — thèmes, ligne rouge, question viscérale du PJ (skill `pc`)
- `RPG/<jeu>/_systeme/{canon,mj}/` — règles du **système de jeu** (format rules-keeper) ; seule référence mécanique de `rpg`. Les sous-systèmes (`RPG/<jeu>/_subsystems/`) sont consommés par `solo-mc` uniquement.
