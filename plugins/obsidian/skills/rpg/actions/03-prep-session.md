...
8. **Écrire** `JDR/<jeu>/campagnes/<campagne>/prep/session-<n>.md`.

## Outputs

`JDR/<jeu>/campagnes/<campagne>/prep/session-<n>.md` (objectif, scènes probables, questions oracle pré-armées, fronts à avancer, tables, accroches PJ). Ne modifie pas `.session-state.yaml` (c'est `solo-mc` qui l'écrit au jeu).
