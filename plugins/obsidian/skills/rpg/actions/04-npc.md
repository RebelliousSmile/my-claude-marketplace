...
1. **Lire le contexte** : `config.yaml` (univers visé, profondeur PNJ attendue), les personnages/factions **canon** (`JDR/<jeu>/univers/<univers>/.docs/canon/`) ET MJ (`.docs/mj/`), le `synopsis.md` et le scénario qui l'invoque. Si le PNJ visé est canon, ne pas le réécrire : créer une fiche MJ qui l'**étend** et le `[[lie]]`.
...
Entrée PNJ dans `JDR/<jeu>/univers/<univers>/.docs/mj/personnages.md` (création MJ) + éventuelles notes de rôle côté campagne. `canon/` inchangé. Marquer les `[À compléter]`.
