...
3. **Rattacher l'univers** : depuis `config.yaml › universe`, cibler `JDR/univers/<univers>/`. Si l'arborescence n'existe pas, créer **deux sous-arbres thématiques** `.docs/canon/` (lore officiel) et `.docs/mj/` (création MJ), chacun avec `terminologie.md`, `factions.md`, `personnages.md`, `histoire.md`, `geographie.md`. S'il existe des sources brutes canoniques, proposer `/writing:lore-extract` pour les consigner **dans `canon/`** ; le contenu créé par le MJ ira dans `mj/` (via `npc`, `faction`, `scenario`).
