...
Lire la prep de campagne (`synopsis.md`, `scenarios/`, `prep/`, `fronts.md`, `config.yaml`), les données d'univers liées (`R/_savoir/univers/<univers>/canon/` et `mj/` : `personnages.md`, `factions.md`, `geographie.md`) et l'`intention.md` du PJ, puis vérifier — signaler par sévérité, corriger sur demande :
...
8. **Système de jeu** (bloquant) — aucune mécanique inventée hors des références du système de jeu.
...
Chaque constat cite un fichier et un correctif ; le verdict est « prêt à jouer » seulement si aucun bloquant ne subsiste (service du PJ, fronts chiffrés, provenance canon/MJ respectée, mécaniques du système de jeu conformes).
