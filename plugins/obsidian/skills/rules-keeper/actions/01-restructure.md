...
> `<systeme-root>` = `R/_savoir/systeme/` ; `<subsys-root>` = `R/_savoir/subsystems/<nom>/`. `R` est le domaine de jeu, découvert localement en remontant jusqu'au marqueur `_savoir/` (voir `jdr-layout.md`).
