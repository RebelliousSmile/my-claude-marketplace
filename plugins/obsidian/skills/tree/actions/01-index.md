...
3. **Infer each domain's effective convention.** For every `subcategory` (domain), look at what exists:
   ...
   - **Unknown domains:** is it dated (`AAAA/MM/...`) or flat? Record `dated` + the observed `convention` string. List its work `units` and any durable areas (`_savoir`, other `_`-prefixed domain-level dirs).
