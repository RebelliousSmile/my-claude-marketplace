...
3. **Build a dry-run plan** — an ordered list of `mv`/rename operations, each as `from → to`, grouped:
   ...
   - **Drift fixes** (default OFF — propose, the user opts in): relocate a unit onto its domain's dated axis, move durable knowledge out of a dated unit into `_savoir/`, etc.
