...
Si `R` ne peut pas être résolu (aucun marqueur `_savoir/` en remontant) : le signaler et proposer d'initialiser `R`, ou écrire dans `<provenance>/` du répertoire courant en avertissant.

> `<univers-root>` = `R/_savoir/univers/<univers>/` — `R` découvert en remontant jusqu'au marqueur `_savoir/` ; `<univers>` déduit du chemin des sources (segment sous `_savoir/univers/`) ou demandé à l'utilisateur.
...
2. **Résoudre le domaine `R` localement** (jamais de chemin global). Partir du répertoire de référence (chemin des sources fourni, sinon CWD), remonter les parents jusqu'au premier dossier contenant le marqueur `_savoir/` : ce dossier est `R`.
   - Déterminer `<univers>` : si les sources sont sous `R/_savoir/univers/<univers>/`, le segment `<univers>` en est déduit ; sinon, le demander à l'utilisateur.
   - Resolve `<univers-root>` = `R/_savoir/univers/<univers>/`
   - Output root = `<univers-root>/<provenance>/` (`canon/` or `mj/`)
   - Si aucun marqueur `_savoir/` n'est trouvé en remontant → la cible n'est pas dans un domaine JDR initialisé : le signaler et proposer d'initialiser `R` (création de `_savoir/`), ou écrire dans `<provenance>/` du répertoire courant en avertissant, puis continuer.
...
Prochaine étape : rafraîchir le manifeste de ressources de R via `obsidian:tree` (cache `R/bank.yml`).
