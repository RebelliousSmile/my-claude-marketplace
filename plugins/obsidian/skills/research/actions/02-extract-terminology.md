...
> The domain `R` is **discovered locally** (walk up from the `univers`/source argument or CWD to the first folder containing the `_savoir/` marker) — no global vault, no per-machine config.
> `extract-terminology` is a canon producer: its output lands in `<univers-root>/canon/terminologie.md` (= `R/_savoir/univers/<univers>/canon/terminologie.md`).
